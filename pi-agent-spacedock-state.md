...
- DONE: Each AC's Verified-by reproduced independently (not trusting self-report): AC1 Pi FO strip, AC2 non-FO no strip
  AC-1: `test_pi_fo_session_renders_spacedock_strip` passes; removed the `toolResult` branch from `tool_result_text` (restored a backup, patched the file), re-ran — `AssertionError` on `assert sd is not None` (transcript_boot returned [], spacedock is None). Restored; passes. AC-2: `test_pi_non_fo_session_has_no_spacedock` passes; replaced the `spacedock_or_none` call with `{"role":"first-officer","workflows":[]}`, re-ran — `AssertionError: {'role': 'first-officer', 'workflows': []} is not None`. Restored; passes.
- DONE: Capture at docs/captures/pi/ confirms toolResult is a message role (independent check of the blocking finding)
  `docs/captures/pi/boot-envelope-fosession-linux.jsonl` record: `toolResult_verdict.is_message_role: true`, `role_value: "toolResult"`, `content_is_list: true`, `content_block_type: "text"`, 223 records; `toolCall_verdict.is_block_type_inside_assistant: true`. README provenance row present.
- DONE: Reviewer's surrogate-path fix independently verified (UnicodeEncodeError/ValueError caught; one strip not the harness)
  `test_a_hostile_envelope_path_never_raises_out_of_the_reader` (cross-platform) and `test_an_unencodable_envelope_path_is_refused` (POSIX) pass. `spacedock_or_none` wraps `session_spacedock` in `try/except (OSError, ValueError)` so a bad envelope costs one strip, not the harness.
- DONE: toolResult branch additive against Claude transcripts (Claude tool_result content blocks unchanged) — independent check
  Standalone script: Claude `role:"tool"` with `type:"tool_result"` blocks extracts `["=== BOOT ===\n{}", "second"]` unchanged; Pi `role:"toolResult"` with `type:"text"` blocks extracts `["=== BOOT ===\n{}"]`; conversation `role:"assistant"` with `type:"text"` blocks returns `[]`. `test_boot_records_require_tool_result_provenance` still passes.
- DONE: Pre-PR suite re-run green from the validation lane: ruff, mypy, lint_embedded, validate_plugins, coverage
  ruff check: All checks passed. ruff format --check: 110 files already formatted. mypy: Success, no issues in 80 files. lint_embedded: clean. validate_plugins: validated. bump_version --current: 0.12.0. coverage: 89.2% (threshold 73%). Full suite: 1188 tests OK (1 skipped). Import graph allowlist test passes.
- DONE: Reviewer findings recorded under workflow labels with a PASSED/REJECTED recommendation
  All three reviewer commits (`f7e61c2` docs, `4e83b79` fix+tests, `917dd56` platform guard) independently confirmed green and load-bearing. Diff purely additive (697 insertions across 21 files since merge-base; PR's own commits touch 5 code/test/doc files). No version fields touched by PR commits. Recommendation: PASSED.

### Summary

Independently reproduced both ACs and their falsifying edits from a fresh validation lane: AC-1 (Pi FO strip) fails when the `toolResult` branch is removed from `tool_result_text`; AC-2 (non-FO no strip) fails when `spacedock` is set unconditionally. The `toolResult` branch is proven additive — Claude `tool_result` content blocks extract unchanged, conversation `text` blocks stay excluded. Surrogate-path fix verified: `_usable_dir` fsencode probe refuses lone surrogates on POSIX, `read_workflow`/`entity_files` catch `ValueError` beside `OSError` cross-platform, and `spacedock_or_none` bounds a bad envelope to one strip. Capture at `docs/captures/pi/` independently confirms `toolResult` is a message role. Pre-PR suite fully green (ruff, mypy, lint_embedded, validate_plugins, coverage 89.2%). Recommendation: PASSED — delivery can proceed to `done`.

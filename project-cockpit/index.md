...
## Stage Report: shaping (cycle 26)

- DONE: Give the existing middle authority cell one truthful state with distinct visual priority.
  Commit `b16342a96c8515fa8262b597ed6e5b2d37ea0986` renders exactly one of `CAPTAIN NEEDED`, `FO INSPECTING`, or `FO CONTINUES`. Exact captain questions receive the alert treatment; recoverable ambiguity receives normal FO treatment with one concrete action; authoritative complete empty attention receives quiet continue treatment without a negative captain sentence. Scan counts and full coverage/source mechanics now exist only under Evidence.
- DONE: Preserve exact authority semantics instead of flattening projected attention into similarly weighted prose.
  Projected `kind` survives the frontend model and drives `AUTHORIZE`, `ANSWER`, `CHOOSE`, and `REVISE`. All exact captain questions render first, at most one FO action remains primary, and `N more FO actions` plus their sources are disclosed under Evidence. A recorded hold/revision without a fresh exact question becomes `APPLY · apply recorded …`, coverage failure becomes a bounded FO scan action, and source unavailability is evidence for `inspect NAME assignment/handoff`; `SOURCE` never appears as an actor.
- DONE: State missing stage linkage only through its proven operational effect and subtract non-actionable primary copy.
  Recovered root work now says `Stage link missing · current work can continue` only when its exact source session is working. A specifically blocked transition renders `Stage link required before <exact next step>` only from an exact `stage_link_required` projection; no blocked step is invented. `Exact operator direction`, source session, and event timestamp moved into Assignment Evidence. Fresh returned-work seconds moved into Execution Evidence, while stale returned work remains qualified in primary copy.
- DONE: Add mutation-sensitive authority and evidence falsifiers.
  Eight adversarial tests cover all three authority states, exact authorize/answer/choose/revise kinds, source-unavailable child evidence, multiple captain/FO rows, failed and incomplete coverage, pending decision history versus a fresh request, nonblocking versus specifically blocking stage linkage, and fresh versus stale returned-work age. The complete cockpit module passes 69/69; the recomputed next-page asset/oracle module passes 21/21; the frontend asset contract passes 12/12.
- DONE: Complete static and proportionate full verification while separating owned regressions from contention and branch baselines.
  Ruff, Ruff format, strict mypy over both changed Python tests, embedded JS/CSS lint, plugin validation, and diff checks pass. Full discovery completed 1,966 tests in 572.075 seconds with five failures, eight errors, and one skip under load that peaked at 122.01. Two owned byte-oracle failures were recomputed from the assets and pass 21/21. The four load-sensitive HTTP/quota cases pass 4/4 alone. Six untouched CapturingServer/contract/Droid baselines reproduce three failures and three errors in isolation, and `test_spacedock` reproduces its Python surrogate-path import error; no cockpit or changed-oracle regression remains.
- DONE: Exercise candidate 8767, inspect the required Chrome frame, and perform a controlled stable swap.
  Candidate 8767 served 422,166 bytes at SHA-256 `400ee5af6d31ebf3b573e6266618b07967c3d22e13ca7b9f8438047cd0c0637a`. Standalone Playwright Chrome captured `/tmp/cargento-mirror-round4-after.png` at 1440×1000; the 65,892-byte image has SHA-256 `edb8829f5c439660f51d6607ad0989ce06f244829c0ec6932ecc06da95813d42`. Inspection confirms `ASSIGNMENT · Restart 5-round review loop`, normal-priority `FO INSPECTING · INSPECT · inspect Harvey assignment`, grouped `Codex · working` execution, only the stale 13-minute return age primary, one compact optional-note action, all four tabs, and no overflow or competing captain/coverage prose.
- DONE: Serve the committed bytes and preserve the authorized Explore-only boundary.
  Candidate 8767 is stopped. Stable `http://127.0.0.1:8766/?next=true#n=project:spacedock-research%2Fcargento` runs PID `2210` with the same bytes and digest. Exact root `codex:01a035ee-2a7b-76f0-873f-eaddc97860c3` is registered at tmux `$4/@4/%10` (`Cargento:1.1`) and renewed read-only. The code commit remains local to `spacedock-ensign/project-cockpit`; no code ref was pushed or integrated, no version or workflow status changed, and `.spacedock/dev` was neither read nor mutated.

### Completion checklist

- DONE: The authority cell exposes exactly one state—CAPTAIN NEEDED, FO INSPECTING, or FO CONTINUES—with distinct priority, and evidence mechanics never outrank the actual next action.
  Eight direct renderer falsifiers exercise the three-state boundary; the inspected live frame shows only normal-priority FO INSPECTING with one named action and its scan/source detail closed under Evidence.
- DONE: Projected attention retains kind and exact compact verbs; source state is evidence, and multiple actions preserve all captain questions plus one FO lead.
  The adversarial four-kind fixture renders AUTHORIZE/ANSWER/CHOOSE/REVISE, pending history routes to FO application until an exact question exists, and the multi-row fixture discloses two additional FO actions without promoting SOURCE.
- DONE: Stage-link copy states only a proven operational effect, while primary copy remains assignment, authority/action, and execution.
  The live screenshot shows `Stage link missing · current work can continue`; a separate exact fixture proves the specifically blocked form, and neither path emits the generic `Workflow stage not linked` label or primary provenance/scan/second-age copy.
